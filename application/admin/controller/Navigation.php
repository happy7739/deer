...
class Navigation extends Admin
{
    ...
    public function index($group = 'admin')
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($modules) {
                ...
                if (false !== $NavigationModel->saveAll($data)) {
                    $this->success('保存成功');
                } else {
                    $this->error('保存失败');
                }
        ...
        // 配置分组信息
        $list_group = NavigationModel::getGroup();
        ...
        if ($group == 'module-sort') {
            $map['status'] = 1;
            $map['pid']    = 0;
            $modules = NavigationModel::where($map)->order('sort,id')->column('icon,title', 'id');
            $this->assign('modules', $modules);
        } else {
            // 获取节点数据
            $data_list = NavigationModel::getMenusByGroup($group);
     ...
     * @throws \Exception
     */
    public function add($module = 'admin', $pid = 0)
    {
        // 保存数据
        if ($this->request->isPost()) {
            ...
            if ($menu = NavigationModel::create($data)) {
                // 自动创建子节点
        ...
        return ZBuilder::make('form')
            ->setPageTitle('新增节点')
            ->addLinkage('module', '所属模块', '', ModuleModel::getModule(), $module, url('ajax/getModelNavigation'), 'pid')
            ->addFormItems([
                ['select', 'pid', '所属节点', '所属上级节点', NavigationModel::getMenuTree(0, '', $module), $pid],
                ['text', 'title', '节点标题'],
                ['radio', 'url_type', '链接类型', '', ['module_admin' => '模块链接(后台)', 'module_home' => '模块链接(前台)', 'link' => '普通链接'], 'module_home']
            ])
            ->addFormItem(
                'text',
                'url_value',
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!NavigationModel::where($map)->find()) {
                NavigationModel::changeModule($data['id'], $data['module']);
            }
        ...
        return ZBuilder::make('form')
            ...
            ->addLinkage('module', '所属模块', '', ModuleModel::getModule(), '', url('ajax/getModelNavigation'), 'pid')
            ->addFormItem('select', 'pid', '所属节点', '所属上级节点', NavigationModel::getMenuTree(0, '', $info['module']))
    ...
    private function setRoleMenu($role_id = '', $roles = [])
    {
        ...
        // 该节点的所有子节点，包括本身节点
        $menu_child   = NavigationModel::getChildsId($role_id);
        $menu_child[] = (int)$role_id;
        // 该节点的所有上下级节点
        $menu_all = NavigationModel::getLinkIds($role_id);
    ...
    public function delete($record = [])
    {
        ...
        $menu = NavigationModel::where('id', $id)->find();

        if ($menu['system_menu'] == '1') $this->error('系统节点，禁止删除');

        // 获取该节点的所有后辈节点id
        $menu_childs = NavigationModel::getChildsId($id);

        // 要删除的所有节点id
        $all_ids = array_merge([(int)$id], $menu_childs);

        // 删除节点
        if (NavigationModel::destroy($all_ids)) {
            Cache::clear();
            // 记录行为
    ...
    private function createChildNode($data = [], $pid = '')
    {
        ...
        if ($child_node) {
            $NavigationModel = new NavigationModel();
            $NavigationModel->insertAll($child_node);
        }
    }
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        if (false !== NavigationModel::where('id', $id)->setField('status', $status)) {
            Cache::clear();
            // 记录行为日志
            if (!empty($record)) {
